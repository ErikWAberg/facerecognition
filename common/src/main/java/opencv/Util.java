...
/*
    public static IplImage multipartFile2Iplimage(MultipartFile file) throws IOException {
        BufferedImage bufferedImage = ImageIO.read(file.getInputStream());
        Frame frame = Converters.javaImageConverter.convert(bufferedImage);
        return Converters.cvImageConverter.convert(frame);
    }

    public static opencv_core.Mat multipartFile2Mat(MultipartFile file, boolean convertToGray) throws IOException {
        BufferedImage bufferedImage = ImageIO.read(file.getInputStream());
        ...
        if (convertToGray) {
            if (iplImage.nChannels() != 3) {
                throw new IOException("Image file has " + iplImage.nChannels() + " != 3 (channels)!");
            }
            IplImage grayiplImage = IplImage.create(frame.imageWidth, frame.imageHeight, opencv_core.IPL_DEPTH_8U, 1);
            cvCvtColor(iplImage, grayiplImage, CV_RGB2GRAY);
            iplImage = grayiplImage;
        }
        frame = iplConverter.convert(iplImage);

        OpenCVFrameConverter.ToMat matConverter = new OpenCVFrameConverter.ToMat();

        return matConverter.convertToMat(frame);
