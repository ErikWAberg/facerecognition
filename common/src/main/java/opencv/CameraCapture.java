...
public class CameraCapture extends Observable {
    ...
    public CameraCapture(final int prefHeight, final int prefWidth, LinkedBlockingQueue<BufferedImage> capturedImageQueue) {
    ...
    private void startCameraCapture() throws FrameGrabber.Exception, InterruptedException {
        ...
        while (captureCanvasFrame.isVisible() && (grabbedFrame = cameraFrameGrabber.grab()) != null) {
            ...
            if (System.currentTimeMillis() - fpsTime > 1000) {
                Log.info("Camera FPS: " + nGrabbedFrames);
                nGrabbedFrames = 0;
                fpsTime = System.currentTimeMillis();
            }
            if (System.currentTimeMillis() - timeSinceSnapshot > ((long) (captureIntervalInSeconds * 1000))) {
                Log.info("Captured image size: <" + grabbedFrame.imageWidth + "x" + grabbedFrame.imageHeight + ">, Depth: " + grabbedFrame.imageDepth + ", Channels: " + grabbedFrame.imageChannels + " prev: " + (System.currentTimeMillis() - prevTime));
